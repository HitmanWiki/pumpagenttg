...
export default function TokensPage() {
  ...
  useEffect(()=>{const t=setTimeout(fetch_,300);return()=>clearTimeout(t)},[search,sort,filter,page])
  ...
  async function fetch_(){
    ...
    if(res.ok){const d=await res.json();setTokens(d.tokens);setTotal(d.pagination.total)}
    setLoading(false)
  }
              ...
              {Array.from({length:8}).map((_,i)=>(
                <div key={i} className="card card-inner">
                  <div style={{display:'flex',alignItems:'center',gap:'0.75rem',marginBottom:'1rem'}}>
                    ...
                    <div style={{flex:1}}><div className="skeleton" style={{height:13,width:'70%',marginBottom:6}}/><div className="skeleton" style={{height:11,width:'40%'}}/></div>
                  </div>
                  <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'0.5rem'}}>
                    <div className="skeleton" style={{height:50,borderRadius:8}}/><div className="skeleton" style={{height:50,borderRadius:8}}/>
                  </div>
                </div>
              ))}
            </div>
  ...
  return(
    <Link href={`/token/${token.id}`} style={{textDecoration:'none',display:'block'}}>
      <div className="card card-inner" style={{cursor:'pointer'}}>
        ...
        <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'0.5rem'}}>
          {[{label:'MARKET CAP',val:fmt(token.marketCapUsd),green:true},{label:'VOL 24H',val:fmt(token.volume24hUsd)}].map(s=>(
