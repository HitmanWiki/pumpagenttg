  ...
  return (
    <>
      ...
      <div className="page">
        ...
        // src/app/dashboard/page.tsx - Update the stat cards and token grid
<div className="section" style={{ paddingTop: 0 }}>
  ...
  <div style={{ 
    ...
    {[
      ...
      <div key={s.label} className="card card-inner" style={{ position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ fontSize: 'clamp(1.5rem, 5vw, 1.75rem)', fontWeight: 800, letterSpacing: '-1px', color: '#00C896', wordBreak: 'break-word' }}>{s.val}</div>
        <div className="mono text-muted" style={{ fontSize: '0.75rem', marginTop: '0.25rem' }}>{s.sub}</div>
      </div>
    ))}
  </div>

  {/* Fee Claim Center - responsive */}
  <div className="card card-inner" style={{ marginBottom: '2rem' }}>
    ...
    <div style={{ marginBottom: '1.5rem' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.8rem', marginBottom: '0.5rem', flexWrap: 'wrap', gap: '0.5rem' }}>
    ...
    <div style={{ marginBottom: '1.5rem' }}>
      ...
      <div style={{ fontSize: 'clamp(2rem, 8vw, 2.5rem)', fontWeight: 800, color: '#00C896', letterSpacing: '-2px', lineHeight: 1, wordBreak: 'break-word' }}>
        {sol(stats.claimableFeesLamports||0)} <span style={{ fontSize: '1rem', color: '#5a7264' }}>SOL</span>
      </div>
    ...
    <button className="btn-primary" onClick={handleClaim} disabled={claiming||claimSol<0.1||!destWallet} style={{ width: '100%', maxWidth: '300px' }}>
      {claiming ? <><span className="spinner"/>Claiming...</> : `Claim ${afterFee.toFixed(4)} SOL`}
    </button>
  </div>

  {/* Tokens - responsive grid */}
  <div>
    ...
    {loading ? (
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: '1rem' }}>
        {[1,2,3,4].map(i=><div key={i} className="card card-inner"><div className="skeleton" style={{ height: 180 }}/></div>)}
      </div>
    ) : tokens.length === 0 ? (
